...
proc = subprocess.Popen(["./venv/bin/python", "-m", "uvicorn", "backend.app.main:app", "--port", "8005"])

try:
    for i in range(15):
        try:
            r = requests.get("http://localhost:8005/api/v1/employees")
    ...
    r = requests.post("http://localhost:8005/api/v1/attendance/upload", files={"file": ("test.csv", csv_content, "text/csv")})
